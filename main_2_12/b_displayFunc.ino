  ...
  if (incomingData[3] == (byte)0x83) {
    ...
    if (buttonAddress >= 0x5000 && buttonAddress <= 0x50F0) {
      ...
      if (!isRunning[selectedChannel - 1] && !isCompleted[selectedChannel - 1]) {
        gotoScreen(0x0001);
        delay(10); //must delay to allow screenchange
        touch(100, 75);
      }
      ...
      //      Serial.print("Selected Channel "); Serial.println(selectedChannel);
    ...
    if (buttonAddress == rotationIntervalAdd) {
